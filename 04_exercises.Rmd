...
```{r}
favorite_stp_by_daniel <- tibble(
  place = c( "Macalester College", "Kyoto Sushi in Roseville", "Kyoto Sushi on Lynsdale","Gaming Goat", "Source Gaming","Buca di Beppo","Tono's", "Italian Pie Shop","Mall of America", "University of St. Thomas"),
  long = c( -93.1712321,-93.1641739, -93.2878205, -93.15614,-93.16711,-93.17687,-93.16725 ,-93.17069,  -93.24258, -93.18976 ),
  lati = c( 44.9378965,45.0054912, 44.9505890,44.9399,45.00311,44.89735,44.93411, 44.93993,44.85448, 44.9448),
  favorite = c("No","Yes","Yes","Yes","No","No","No","No", "No","No")
  )
...
```{r}
pal <- colorFactor(palette = c("red","blue"), domain = favorite_stp_by_daniel$favorite)

leaflet(favorite_stp_by_daniel) %>% 
  addTiles() %>% 
  addPolylines(data = favorite_stp_by_daniel, lat = ~lati, lng=~long ) %>% 
  addCircles(lat = ~lati, lng=~long,label = ~str_to_title(place), color = ~pal(favorite))
```
...
```{r}
pal <-  colorFactor("Greens", domain = MplsStops$problem)

leaflet(MplsStops) %>% 
  addCircleMarkers(data = MplsStops,lat = ~lat,lng = ~long, stroke = FALSE, fillColor  = ~pal(problem),radius = .5)
```  
...
```{r}
pal2 <- colorNumeric("plasma", mpls_all$sus, reverse =TRUE)

leaflet(mpls_all) %>% 
  addTiles() %>% 
  addPolygons(label = ~str_to_title(BDNAME), fillColor = ~pal2(sus), fillOpacity = .7) %>% 
...
* It show the south east neighborhood have a lot more suspicious activity  
  
  18. Use `leaflet` to create a map of your own choosing. Come up with a question you want to try to answer and use the map to help answer that question. Describe what your map shows. 

* What is the college graduation rate of each neighborhood?

```{r}
pal2 <- colorNumeric("plasma", mpls_all$collegeGrad, reverse =TRUE)

leaflet(mpls_all) %>% 
  addTiles() %>% 
  addPolygons(label = ~str_to_title(BDNAME), fillColor = ~pal2(collegeGrad), fillOpacity = .7) %>% 
  addLegend(position = "bottomright",
            pal = pal2, 
            values = ~collegeGrad,
...
* The map shows that the south west neighborhoods have a higher graduation rate.  
